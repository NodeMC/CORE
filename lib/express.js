...
module.exports = class Express {
  ...
  constructor(app, server, ver, log) {
    ...
    passport.use("api-auth", new Hawk(async (username, done) => {
      try {
        const user_cursor = await db.find("users", "username", username)
    ...
    this.mountRoutes(app, path.join(__dirname, "../backend/routes/"), "v" + semver.major(ver), {
      requiresAuth:   passport.authenticate("api-auth", { session: false }),
      server: server,
      passport: passport
    });
  }
