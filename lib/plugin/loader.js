...
module.exports = class PluginLoader {
     ...
     * 
     * @param {any} pluginId The ID of the plugin to execute.
     * @returns {undefined}
     */
     ...
     */
    loadAll(dir = "plugins/") {
        let pluginsDir = path.dirname(require.main.filename);
        let modules = fs.readdirSync(pluginsDir);
        for (let pluginModule in modules) {
            if ({}.hasOwnProperty.call(modules, pluginModule)) {
                this.loadPlugin(path.join(pluginsDir, pluginModule));
            }
        }
    }
}
